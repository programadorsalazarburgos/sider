...
exports.format = {
    ...
    "format escape brackets": function (test) {
        ...
        test.equal(b.format('[LLL] LLL'), 'LLL Febrero 14 2009 3:25 PM', 'localized tokens with escaped localized tokens (recursion)');
    ...
    "isDST": function (test) {
        ...
        if (janIsDst && julIsDst) {
            test.ok(0, 'Enero and Julio cannot both be in DST');
            test.ok(0, 'Enero and Julio cannot both be in DST');
        } else if (janIsDst) {
            test.ok(jan1.isDST(), 'Enero 1 is DST');
            test.ok(!jul1.isDST(), 'Julio 1 is not DST');
        } else if (julIsDst) {
            test.ok(!jan1.isDST(), 'Enero 1 is not DST');
            test.ok(jul1.isDST(), 'Julio 1 is DST');
        } else {
            test.ok(!jan1.isDST(), 'Enero 1 is not DST');
            test.ok(!jul1.isDST(), 'Julio 1 is not DST');
        }
