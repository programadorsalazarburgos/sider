SeriesApp.controller("CreateInventarioColegioCtrl", function($scope, GrupoService, fileUpload, $http, $location, base_api, $q){
$scope.title = "Agregar Inventario Colegio";
...
$scope.person = {};
$http.get(base_api + "/obtener/implementos")
.success(function(response){
  $scope.implementos = response;
...
$scope.getVal=function(){
    ...
    {
        $scope.requiredCantidad = true;
        $scope.requiredQuitar = false;
        $scope.cantidadEliminar = null;
    }
    else
    {
        $scope.requiredCantidad = false;
        $scope.requiredQuitar = true;
...
$scope.selectedSede = function()
{
  ...
  $http.get(base_api + "/obtener/implementos/" + sede)
    .success(function(response){
    $scope.list = response;
...
$scope.formsubmit = function(isValid) {     
      ...
      var obt_sede = $scope.person.id;  
      var sede = obt_sede['id'];
      var implemento = $scope.implemento;
      ...
      $.ajax({
        url: base_api + '/inventariocolegio/create',
        type: 'POST',
        dataType: 'JSON',
        data: {
          sede: sede, 
