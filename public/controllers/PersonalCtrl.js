SeriesApp.controller("PersonalCtrl", function($scope, GrupoService, $routeParams, fileUpload, $http, $location, $timeout, base_api, $q){
      ...
      $http.get(base_api + "/admin/postpersona/cargar/" + no_documento_beneficiario)
        .success(function(response){
          ...
          .success(function(EstratoVereda){
                ...
                $scope.comuna = EstratoVereda.comuna_id;
          ...
          //Universidad
          $scope.universidad = $scope.empleado.nombre_universidad; 
          //Nivel de escolaridad 
          $scope.tipo_esc = {
          'id': 1,
          'unit': $scope.empleado.escolaridad_id
          }

          //Estado Escolaridad
          $scope.estado_esc = {
            'id': 1,
            'unit': $scope.empleado.estado_escolaridad
          }

          //Ocupacion Actual
          $scope.tipo_ocup = {
            'id': 1,
            'unit': $scope.empleado.ocupacion_id
            }
  ...
  $scope.toggleSelectionDisciplina = function toggleSelectionDisciplina(seleccion)
  {
    ...
    if (index > -1)
    {
      ...
      $http({
        method: 'POST',
        type: 'JSON',
        url: base_api + "/eliminar/poblacional/" + $scope.empleado.ficha_save,
        data: $.param({
            grupo_pcs: seleccion.id
