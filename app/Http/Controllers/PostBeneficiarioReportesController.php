...
class PostBeneficiarioReportesController extends Controller
{
...
//    $this->middleware('permission:ver-roles', ['only' => 'vista']);
	...
	public function vista(){
		...
		return view("postreportegeneral.appreportegeneral");
	}

	public function vistaDetallado(){

		return view("postreportedetallado.appreportedetallado");
	}
...
$beneficiario = FichaComunidad::findOrfail($id);
	...
	public function BeneficiariosReporteDetallado()
  {
    ...
    WHERE `tbl_cm_ficha`.`tenantId` = '.$wheres.'
    GROUP BY
    `tbl_gen_asistencias`.`ficha_id`');
  ...
  public function total_beneficiarios(Request $request)
  {


  	  $datos = FichaComunidad::whereBetween('tbl_cm_ficha.fecha_inscripcion', [$request->entre, $request->hasta])
  	  						   ->where('tbl_cm_ficha.tenantId', '=', Auth::user()->tenantId)
  	  						   ->where('tbl_cm_ficha.grupo_id', '!=', null)
  	  						   ->count();
